...
export function setupRoadmapCarousel(container: HTMLElement, colCount: number): void {
  ...
  const dots = Array.from({ length: colCount }, () =>
  ...
  function applyMobile(): void {
    container.querySelectorAll<HTMLElement>(".vzd-roadmap-col").forEach((c, i) =>
      c.classList.toggle("vzd-roadmap-col--active", i === current)
    );
    dots.forEach((d, i) => d.classList.toggle("is-active", i === current));
    prev.disabled = current === 0;
    next.disabled = current === colCount - 1;
  }

  function resetLayout(): void {
    container.querySelectorAll<HTMLElement>(".vzd-roadmap-col").forEach(c =>
      c.classList.remove("vzd-roadmap-col--active")
    );
  ...
  next.addEventListener("click", () => { if (current < colCount - 1) { current++; applyMobile(); } });
  ...
  const onTouchEnd = (e: TouchEvent): void => {
    ...
    if (Math.abs(delta) > SWIPE_THRESHOLD_PX) {
      if (delta > 0 && current < colCount - 1) { current++; applyMobile(); }
...
export function setupPaceLayerCarousel(container: HTMLElement, rowCount: number): void {
  ...
  const dotsWrap = nav.createEl("div", { cls: "vizardry-nav-dots" });
  const dots = Array.from({ length: rowCount }, () =>
    dotsWrap.createEl("span", { cls: "vizardry-nav-dot" })
  );
  ...
  function applyMobile(): void {
    container.querySelectorAll<HTMLElement>(".vzd-pl-row").forEach((r, i) =>
      r.classList.toggle("vzd-pl-row--active", i === current)
    );
    dots.forEach((d, i) => d.classList.toggle("is-active", i === current));
    prev.disabled = current === 0;
    next.disabled = current === rowCount - 1;
  }

  function resetLayout(): void {
    container.querySelectorAll<HTMLElement>(".vzd-pl-row").forEach(r => {
      r.classList.remove("vzd-pl-row--active");
    });
    dots.forEach(d => d.classList.remove("is-active"));
    prev.disabled = false;
    next.disabled = false;
  ...
  prev.addEventListener("click", () => { if (current > 0) { current--; applyMobile(); } });
  next.addEventListener("click", () => { if (current < rowCount - 1) { current++; applyMobile(); } });
  ...
  const onTouchEnd = (e: TouchEvent): void => {
    ...
    const delta = touchStartX - e.changedTouches[0].clientX;
    if (Math.abs(delta) > SWIPE_THRESHOLD_PX) {
      if (delta > 0 && current < rowCount - 1) { current++; applyMobile(); }
      else if (delta < 0 && current > 0) { current--; applyMobile(); }
    }
  };
  container.addEventListener("touchstart", onTouchStart, { passive: true });
